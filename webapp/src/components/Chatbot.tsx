...
declare global {
  // Browser speech APIs aren't typed in TS by default.
  interface Window {
...
interface Props {
  email: string;
  onSuggestion: (topic?: string | null, value?: number | null, unit?: string | null) => void;
  suggestion: Suggestion;
  onGenerateCourse: () => Promise<void>;
  generatingCourse: boolean;
  courseAnnouncement?: CourseAnnouncement | null;
}
...
const Chatbot = ({
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      role: "assistant",
      content:
        "Hey! I'm your personal learning coach. Tell me the topic you want to master and how many days or hours you can dedicate. I can listen to your voice or text.",
    },
  ]);
  ...
  const handleSend = async (event?: FormEvent) => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Chat server appears offline.";
      setError(message);
    } finally {
      setLoading(false);
    }
  ...
  const handleCourseClick = async () => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Unable to generate the course right now.";
      setError(message);
    }
  };

  const buildCourseLink = (value?: string | null) => {
  ...
  return (
    <section className="card">
      <h3 style={{ marginTop: 0 }}>Interactive chatbot</h3>
      <div
        style={{
          maxHeight: 320,
          overflowY: "auto",
          ...
          >
            ...
            {message.topic && (
              <div className="pill" style={{ marginTop: 8 }}>
                topic hint: {message.topic}
              </div>
            )}
            {message.duration_value && (
              <div className="pill" style={{ marginTop: 8 }}>
                time hint: {message.duration_value} {message.duration_unit}
              </div>
            )}
            {message.course_url && (
              <a
                href={buildCourseLink(message.course_url) ?? "#"}
                target="_blank"
                rel="noreferrer"
                style={{
      ...
      >
        <div className="input-with-voice">
          <input
            type="text"
            placeholder="Ask for a plan, e.g., 'I need to learn GenAI in 2 hours'"
            value={input}
      ...
      {error && (
        <p style={{ color: "#dc2626", fontSize: "0.9rem", marginTop: 8 }}>
          {error}
        </p>
      )}
    </section>
